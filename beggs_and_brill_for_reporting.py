...
def beggs_and_brill(P,T,liquid_rate, WC, GOR, gas_grav, oil_grav,
    ...
    qo = Bo * oil_rate / 15387                                 #Oil flowrate in ft_/s
    qw = Bw * wor * oil_rate / 15387                           #Water flowrate in ft_/s
    ql = qo + qw                                               #Liquid flowrate in ft_/s
    ...
    if Nre<2000:
            ...
            f=(1/(-2*math.log(constant)))   #Calculating the first f value with Re and roughness
            
            def error(a1,a2):   #Function that finds the error between 2 numbers
                return abs(a1-a2)/a1*100
            
            list=[]   
            list.append(round(f0,5))  #Adding the first random f value
            list.append(round(f,5))    
            for i in range(10):
...
print(f"Oil Flow Rate: {qo} bbl/day\n")
print(f"Water Flow Rate: {qw} bbl/day\n")
print(f"Gas Flow Rate: {qg} scf/day\n")
